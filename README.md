...
![ロボットアームとゲームパッド](docs/images/hardware.jpg)
...
> **補足：MAC アドレスについて**  
> `main.py` に書き込まれた MAC アドレスは開発で使ったゲームパッドの値。別のゲームパッドを使う場合は `tools/scan.py` を Pico W で実行してゲームパッドの MAC アドレスを確認し、`main.py` を書き換えてから転送する。

---

## 使い方

COWBOX T-12 を **X ボタン + HOME ボタン長押し**でペアリングモードに入れてから、Pico W に電源を投入する。
...
| 左スティック 上下 | アーム (Elbow)   | アームを伸ばし / かき込み  |
| 右スティック 上下 | ブーム (Shoulder)| ブームを上げ / 下げ        |
| L ボタン          | ハンド (Hand)    | ハンド左回転               |
| R ボタン          | ハンド (Hand)    | ハンド右回転               |
| B ボタン          | 全サーボ         | 初期位置にリセット         |

![操作方法](docs/images/operation.jpg)
...
別のゲームパッドを使う場合は MAC アドレスを確認して書き換える。
...
> **補足：サーボ設定の速度との違い**  
> `速度 [us/20ms]` はサーボが目標角度に追従する物理的な速さ、`DUTY_STEP` は操作入力に対して目標角度がどれだけ変化するかを表す。  
...
ゲームパッドで Raspberry Pi Pico W を操作する部分は、以下の動画を参考にさせていただきました。
